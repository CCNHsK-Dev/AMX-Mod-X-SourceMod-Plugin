...
/*================================================================================
		...
		*****************************************************
		***** [Zombie Plague Mod 4.3 ReMiX V1.0 Beta1] ******
		*****************************************************
	...
	Zombie Plague Mod ReMiX Mod
	2011 Remix Ver by HsK            Copyright (C) by MeRcyLeZZ
	...
	* v1.0 beta1: (17/4/2011)
	   - cvar can set item cost
	   - Added: Human classes, Countdown MSG, player have a frozen can damage her, 
                   Armor can withstand infect bomb , Survivor will infected, 
                   Fire bomb can kill zombie, Can set zombie sound, Can set class name, 
                   Have new native can set player model and in the server will not lag A_A,
		   Add GoD Mode, Change Aim MSG

	   - 	    人類種族, 倒數字體, 冰涷時可被攻擊, 護甲可抵抗感染炸彈, 倖存者不被感染, 
		   火炎彈可殺死喪屍, 可獨立設定喪屍音效, 可獨立設定種族名稱, 新增指令可設定角色模組, 
		   新增無敵效果, 取消購買區域, 修改喪屍血量顏色, 描準信息更改, 可使用cvar 設定道具價格
...
new g_fire_ing[33][2] // if fire 
new g_humanclass[33] // human class
new g_humanclassnext[33] // human class for next infection
new bool:g_NSPY[33] // Get have use natives set player model
new g_NSPYmodel[33][32] // natives set player model
new g_GoD_mode[33] // ZP godmode ^^
...
g_msgStatusText, g_msgStatusValue
...
g_fwUserLastZombie, g_fwUserLastHuman, g_fwDummyResult, g_fwUserHumanized_class_post, 
g_feUserfrozenpost, g_feUserfirepost
...
Array:g_hclass2_modelindex, Array:g_hclass2_hp, Array:g_hclass2_spd, 
...
model_vknife_nemesis[64], model_vm249_survivor[64], model_velite_survivor[64]
model_grenade_infect[64], model_grenade_fire[64], model_grenade_frost[64], 
model_grenade_flare[64], model_vknife_admin_human[64], model_vknife_admin_zombie[64],
...
Array:sound_countdown, Array:g_zclass_sound1, Array:g_zclass_sound2, Array:g_zclass_sound3
...
cvar_survweapon, cvar_survweapon2, cvar_plaguesurvnum, cvar_infectionscreenfade, 
...
cvar_nemknockback, cvar_nvgcolor[3], cvar_nemnvgcolor[3], cvar_humnvgcolor[3], 
cvar_flashcolor[3], cvar_hudicons, cvar_respawnzomb, cvar_respawnhum, cvar_respawnnem, 
cvar_respawnsurv, cvar_startammopacks, cvar_randweapons, cvar_antidotelimit, cvar_madnesslimit,
cvar_adminknifemodelshuman, cvar_adminknifemodelszombie, cvar_keephealthondisconnect,
cvar_countdown_msg, cvar_frozen_damage, cvar_infectbomb_armor, cvar_infectbomb_damage,
cvar_infect_survivor, cvar_fire_kill, cvar_hclasses, cvar_zclasses_ch, cvar_set_blood,
cvar_blood_color

new cvar_pcvar_get[200], get_have_cvar[200]   //ITEM Pcvar get
new get_zclass_sound[200]
...
new g_set_classname[33][32] // Set class name
new g_show_zclass_m[33] // Show Zombie class meun
...
public plugin_natives()
{
	...
	register_native("zp_get_user_fire", "native_get_user_fire", 1)
	// Class Name
	register_native("zp_get_user_class_name", "native_get_user_class_name", 1)
	register_native("zp_set_user_class_name", "native_set_user_class_name", 1)

	register_native("zp_set_player_model", "native_set_player_model", 1)
	register_native("zp_register_zombie_sound", "native_register_zombie_sound", 1)
	// Human Class
	...
	register_native("zp_get_user_godmode", "native_get_user_godmode", 1)
	register_native("zp_set_user_godmode", "native_set_user_godmode", 1)
}
...
public plugin_precache()
{
	...
	sound_countdown = ArrayCreate(64, 1)
	g_zclass_sound1 = ArrayCreate(32, 1)
	g_zclass_sound2 = ArrayCreate(32, 1)
	g_zclass_sound3 = ArrayCreate(32, 1)
	...
	engfunc(EngFunc_PrecacheModel, model_velite_survivor)
...
public plugin_init()
{
	...
	cvar_survweapon2 = register_cvar("zp_surv_weapon", "weapon_elite")
	...
	// CVARS - New CvarS	 hsk
	cvar_countdown_msg = register_cvar("zp_countdown_msg", "1")
	cvar_frozen_damage = register_cvar("zp_frozen_damage", "1")
	cvar_infectbomb_armor = register_cvar("zp_infectbomb_armor", "1")
	cvar_infectbomb_damage = register_cvar("zp_infectbomb_damage", "80")
	cvar_infect_survivor = register_cvar("zp_infect_survivor", "0")
	cvar_fire_kill = register_cvar("zp_fire_kill", "1")
	cvar_hclasses = register_cvar("zp_hclasses", "1")
	cvar_zclasses_ch = register_cvar("zp_zclasses_ch", "1")
	cvar_set_blood = register_cvar("zp_set_blood", "1")
	cvar_blood_color = register_cvar("zp_blood_color", "182")
	...
	g_fwUserHumanized_class_post = CreateMultiForward("zp_user_humanized_class_post", ET_IGNORE, FP_CELL)
	g_feUserfrozenpost = CreateMultiForward("zp_user_frozen_post", ET_IGNORE, FP_CELL)
	g_feUserfirepost = CreateMultiForward("zp_user_fire_post", ET_IGNORE, FP_CELL)
...
public event_round_start()
{
	...
	g_countdown_timE = floatround(get_pcvar_float(cvar_warmup))
...
{
	aim_text(id, read_data(2), g_zombie[read_data(2)])
}
...
public aim_text(id, target, zombie)
{
	static text[100], magtext[100]

	if (!(equal(g_set_classname[target], "0")))
		formatex(text, sizeof text - 1, "%s", g_set_classname[target])
	else
	{
		if (zombie == 1)
		{
			if (g_nemesis[target])
				formatex(text, sizeof text - 1, "%L", id, "CLASS_NEMESIS")
			else
				formatex(text, sizeof text - 1, "%s", g_zombie_classname[target])
		}
		else
		{
			if (g_survivor[target])
				formatex(text, sizeof text - 1, "%L", id, "CLASS_SURVIVOR")
			else 
			{
				if (g_open_gclass)
					formatex(text, sizeof text - 1, "%s", g_human_classname[target])
				else
					formatex(text, sizeof text - 1, "%L", id, "CLASS_HUMAN")
			}
		}
	}
	...
	{
		add(text, sizeof text - 1, " : %p2 %h: %i3%%%")
		if (!g_zombie[target])
			format(magtext, 99, "%s Armor: %d%%%", text, get_user_armor(target));
		else
	...
	message_begin(MSG_ONE,g_magStatusValue,_,id)
	write_byte(3)
	write_short(get_user_health(target))
	message_end()
}
...
public fw_PlayerSpawn_Post(id)
{
	...
	if (g_open_gclass && !g_survivor[id] && !g_zombie[id]) set_hclass_A(id)
...
public fw_TakeDamage(victim, inflictor, attacker, Float:damage, damage_type)
{
	...
	if (!get_pcvar_num(cvar_frozen_damage) && g_frozen[victim])
		return HAM_SUPERCEDE;
	...
	if (!g_zombie[attacker])
	{
		...
		if (get_pcvar_num(cvar_set_blood)) set_blood_color(victim)
	...
	if (!get_pcvar_num(cvar_infect_survivor) && g_survivor[victim])
		return HAM_IGNORED;

	if (g_GoD_mode[victim] == 2)	
		return HAM_IGNORED;
...
{
	if (!get_pcvar_num(cvar_set_blood))
		return HAM_IGNORED;

	SetHamReturnInteger(get_pcvar_num(cvar_blood_color))
	return HAM_SUPERCEDE;
}

public set_blood_color(id)
{
	...
	switch(hitpoint)
	{
		...
		case 2: origin[2] += random_num(23, 27)
		case 3: origin[2] += random_num(8, 12)
		case 4:
		{
			origin[2] += random_num(8, 12)
			origin[0] += random_num(4, 6)
			origin[1] += random_num(4, 6)
		}
		case 5:
		{
			origin[2] += random_num(8, 12)
	...
	for (i = 0; i <= 2; i++)
	{
		...
		write_short(g_bloodSpr)
		write_byte(get_pcvar_num(cvar_blood_color))
		write_byte(j)
		message_end()
	}
}
...
public fw_EmitSound(id, channel, const sample[], Float:volume, Float:attn, flags, pitch)
{
	...
	if (sample[7] == 'b' && sample[8] == 'h' && sample[9] == 'i' && sample[10] == 't')
	{
		...
		{
			if (get_zclass_sound[g_zombieclass[id]])
				ArrayGetString(g_zclass_sound1, g_zombieclass[id], sound, charsmax(sound))
			else
	...
	{
		if (get_zclass_sound[g_zombieclass[id]]) ArrayGetString(g_zclass_sound2, g_zombieclass[id], sound, charsmax(sound))
		else ArrayGetString(zombie_die, random_num(0, ArraySize(zombie_die) - 1), sound, charsmax(sound))
		emit_sound(id, channel, sound, volume, attn, flags, pitch)
		return FMRES_SUPERCEDE;
	}
	...
	{
		if (get_zclass_sound[g_zombieclass[id]]) ArrayGetString(g_zclass_sound3, g_zombieclass[id], sound, charsmax(sound))
		else ArrayGetString(zombie_fall, random_num(0, ArraySize(zombie_fall) - 1), sound, charsmax(sound))
		emit_sound(id, channel, sound, volume, attn, flags, pitch)
		return FMRES_SUPERCEDE;
	}
...
public fw_ClientUserInfoChanged(id)
{
	...
	if (!g_handle_models_on_separate_ent)
	{
		...
		if (((!g_NSPY[id] && !equal(currentmodel, g_playermodel[id]))
		|| g_NSPY[id] && !equal(currentmodel, g_NSPYmodel[id])) && !task_exists(id+TASK_MODEL))
...
public fw_PlayerPreThink(id)
{
	...
	else
	{
		...
		else
		{
			...
			{
				if (!g_open_gclass) set_pev(id, pev_maxspeed, g_cached_humanspd)
				else set_pev(id, pev_maxspeed, g_human_spd[id])
			}
		}
	}
...
show_menu_game(id)
{
	...
	// 4. Human class
	if (g_open_gclass)
		len += formatex(menu[len], charsmax(menu) - len, "\r4.\w Human Class^n")
	else
		len += formatex(menu[len], charsmax(menu) - len, "\d4. Human Class^n")
		...
		case 1: // Extra Items
		{
			...
			if (get_pcvar_num(cvar_extraitems))
			{
				...
				if (g_isalive[id])
				{
					...
					{
						if (!get_have_cvar[item]) continue;

						ArraySetCell(g_extraitem_cost, item, get_pcvar_num(cvar_pcvar_get[item]))
					//	ArrayPushCell(g_extraitem_cost, get_pcvar_num(cvar_pcvar_get[item]))
					}
...
buy_extra_item(id, itemid, ignorecost = 0)
{
	...
	if (!ignorecost)
	{
		new temp = ArrayGetCell(g_extraitem_cost, itemid)

		// Check that we have enough ammo packs
		if (g_ammopacks[id] < temp)
		{
			zp_colored_print(id, "^x04[ZP]^x01 %L", id, "NOT_ENOUGH_AMMO")
			return;
		}
...
public menu_zclass(id, menuid, item)
{
	...
	if (g_show_zclass_m[id] && get_pcvar_num(cvar_zclasses_ch) && g_zombie[id])
		zombieme(id, 0, 0, 0, 0)
...
public menu_hclass(id, menuid, item)
{
	...
	// Show selected zombie class info and stats
	zp_colored_print(id, "^x04[ZP]^x01 %L: %s", id, "HUMAN_SELECT", name)
	zp_colored_print(id, "^x04[ZP]^x01 %L: %d %L: %d %L: %d %%", id, "HUMAN_ATTRIB1", ArrayGetCell(g_hclass_hp, g_humanclassnext[id]), id, "HUMAN_ATTRIB2", ArrayGetCell(g_hclass_spd, g_humanclassnext[id]),
...
public message_clcorpse()
{
	...
	set_msg_arg_string(1, g_NSPY[id] ? g_NSPYmodel[id] : g_playermodel[id])
}
...
public message_statusIcon(msg_id, msg_dest, id)
{
        if(is_user_alive(id))
        {
                ...
                if(equal(szMsg, "buyzone") && get_pcvar_num(cvar_removemoney))
                        return PLUGIN_HANDLED
        }
        return PLUGIN_CONTINUE
} 
...
{
	if (g_countdown_timE >= 1)
	{
		static sound[64]
		if (get_pcvar_num(cvar_countdown_msg))
		{
			set_hudmessage(255, 255, 255, HUD_EVENT_X, HUD_EVENT_Y + 0.2, 0, 0.0, 3.0, 2.0, 1.0, -1)
			ShowSyncHudMsg(0, g_MsgSync, "%L", LANG_PLAYER, "COUNTDOWN_TIME_MSG", g_countdown_timE)
		}
		else
			client_print(0, print_center,"%L", LANG_PLAYER, "COUNTDOWN_TIME_MSG", g_countdown_timE)

		new have_sound = ArraySize(sound_countdown)
		if (have_sound >= g_countdown_timE)
		{
			ArrayGetString(sound_countdown, g_countdown_timE-1, sound, charsmax(sound))
			PlaySound(sound)
		}
...
{
	if (!get_pcvar_num(cvar_infect_survivor) && g_survivor[id] && infector)
		return;
	...
	if (!nemesis && get_pcvar_num(cvar_zclasses_ch))
	{
		if (!g_show_zclass_m[id])
		{
			g_show_zclass_m[id] = true
			set_task(0.2, "show_menu_zclass", id)
			set_task(6.0, "set_cannot_show_set", id)
			if (g_isbot[id])
			{
				g_zombieclassnext[id] = random_num(0, g_zclass_i - 1)
				zombieme(id, 0, 0, 0, 0)
	...
	if (rewards && infector)
	{
		...
		SendDeathMsg(infector, id, 1, "infection")
		FixDeadAttrib(id)
	...
	copy(g_set_classname[id], charsmax(g_set_classname[]), "0")
	...
	else
	{
		...
		else
		{
			...
			{
				new temp1 = ArrayGetCell(g_zclass_modelsstart, g_zombieclass[id])
				new temp2 = ArrayGetCell(g_zclass_modelsend, g_zombieclass[id])
...
humanme(id, survivor, silentmode)
{
	...
	if (survivor)
	{
		...
		static survweapon[32], survweapon2[32]
		...
		ExecuteHamB(Ham_GiveAmmo, id, MAXBPAMMO[cs_weapon_name_to_id(survweapon2)], AMMOTYPE[cs_weapon_name_to_id(survweapon2)], MAXBPAMMO[cs_weapon_name_to_id(survweapon2)])
	...
	if (g_open_gclass && !g_survivor[id])
		set_hclass_A(id)
...
public set_cannot_show_set(id) g_show_zclass_m[id] = false
...
set_hclass_A(id)
{
	...
	if (g_isbot[id]) g_humanclassnext[id] = random_num(0, g_hclass_i)

	g_humanclass[id] = g_humanclassnext[id]

	g_human_spd[id] = float(ArrayGetCell(g_hclass_spd, g_humanclass[id])) // set speed
	ArrayGetString(g_hclass_name, g_humanclass[id], g_human_classname[id], charsmax(g_human_classname[]))
	fm_set_user_health(id, floatround(float(ArrayGetCell(g_hclass_hp, g_humanclass[id]))))
	...
	ExecuteForward(g_fwUserHumanized_class_post, g_fwDummyResult, id)
}
...
public cache_cvars()
{
	...
	g_open_gclass = get_pcvar_num(cvar_hclasses)
	if (!g_hclass_i && g_open_gclass) g_open_gclass = 0
}
	...
	while (file && !feof(file))
	{
		...
		switch (section)
		{
			...
			case SECTION_WEAPON_MODELS:
			{
				...
				else if (equal(key, "V_ELITE SURVIVOR"))
					copy(model_velite_survivor, charsmax(model_velite_survivor), value)
...
save_customization()
{
	...
	for (i = 0; i < size; i++)
	{
		if (ArrayGetCell(g_zclass_new, i))
		{
			...
			new temp1 = ArrayGetCell(g_zclass_modelsstart, i)
			new temp2 = ArrayGetCell(g_zclass_modelsend, i)
	...
	for (i = 0; i < size; i++)
	{
		if (ArrayGetCell(g_hclass_new, i))
		{
			...
			new temp1 = ArrayGetCell(g_hclass_modelsstart, i)
			new temp2 = ArrayGetCell(g_hclass_modelsend, i)
...
infection_explode(ent)
{
	...
	while ((victim = engfunc(EngFunc_FindEntityInSphere, victim, originF, NADE_EXPLOSION_RADIUS)) != 0)
	{
		...
		if (get_pcvar_num(cvar_infectbomb_armor))
		{
			new Float:victim_armor
			pev(victim, pev_armorvalue, victim_armor)
			if (victim_armor >= float(cvar_infectbomb_damage))
			{
				engfunc(EngFunc_EmitSound, victim, CHAN_BODY, sound_armorhit, 1.0, ATTN_NORM, 0, PITCH_NORM)
				set_pev(victim, pev_armorvalue, floatmax(0.0, victim_armor - float(cvar_infectbomb_damage)))

				continue;
			}
			set_pev(victim, pev_armorvalue, floatmax(0.0, 0.0))
		}
...
fire_explode(ent)
{
	...
	while ((victim = engfunc(EngFunc_FindEntityInSphere, victim, originF, NADE_EXPLOSION_RADIUS)) != 0)
	{
		...
		g_fire_ing[victim][0] = true
		g_fire_ing[victim][1] = attacker

		ExecuteForward(g_feUserfirepost, g_fwDummyResult, victim)
...
frost_explode(ent)
{
	...
	while ((victim = engfunc(EngFunc_FindEntityInSphere, victim, originF, NADE_EXPLOSION_RADIUS)) != 0)
	{
		...
		ExecuteForward(g_feUserfrozenpost, g_fwDummyResult, victim)
...
replace_weapon_models(id, weaponid)
{
	switch (weaponid)
	{
		...
		case CSW_ELITE:  // Survivor's ELITE
		{
			if (g_survivor[id])
				set_pev(id, pev_viewmodel2, model_velite_survivor)
		}
...
public ShowHUD(taskid)
{
	...
	else // humans
	{
		...
		{
			if (g_hclass_i && g_open_gclass) copy(class, charsmax(class), g_human_classname[id])
			else formatex(class, charsmax(class), "%L", ID_SHOWHUD, "CLASS_HUMAN")
		}
	}
	
	if (!(equal(g_set_classname[id], "0")))
		copy(class, charsmax(class), g_set_classname[id])
...
public native_register_extra_item(const name[], cost, team, use_cvar, const pcvar_get[])
{
	// ZP disabled
	if (!g_pluginenabled)
		return -1;
	...
	get_have_cvar[g_extraitem_i] = 0
	
	// Add the item
	ArrayPushString(g_extraitem_name, name)
	if (!use_cvar) ArrayPushCell(g_extraitem_cost, cost)
	else 
	{
		get_have_cvar[g_extraitem_i] = 1
		cvar_pcvar_get[g_extraitem_i] = get_cvar_pointer(pcvar_get)
		ArrayPushCell(g_extraitem_cost, 0)
	}
...
// Native zp_get_user_fire(id)
public native_get_user_fire(id)
{
	return g_fire_ing[id][0];
}

public native_get_user_class_name(id, name[256]) 
{    
	param_convert(2)

	if (g_zombie[id]) 
	{
		if (g_nemesis[id]) formatex(name, charsmax(name), "%L", id, "CLASS_NEMESIS");
		else copy(name, charsmax(name), g_zombie_classname[id]);
	} else {
		if (g_survivor[id]) formatex(name, charsmax(name), "%L", id, "CLASS_SURVIVOR");
		else
		{
			if (!g_open_gclass) formatex(name, charsmax(name), "%L", id, "CLASS_HUMAN")
			else copy(name, charsmax(name), g_human_classname[id])
		}
	}
	return id;
}

public native_set_user_class_name(id, const name[])
{
	param_convert(2)

	copy(g_set_classname[id], charsmax(g_set_classname[]), name)

	return -1;
}

public native_set_player_model(id, const model_name[])
{
	param_convert(2)
	...
	{
		g_NSPY[id] = true
		copy(g_NSPYmodel[id], charsmax(g_NSPYmodel[]), model_name)

		set_task(0.7, "fm_cs_set_user_model", id+TASK_MODEL)
	}

	return -1;
}

// Native: zp_register_zombie_sound
public native_register_zombie_sound(zombie_class, const bhit[], const dieea[], const fall[])
{
	if (!g_pluginenabled)
		return -1;
	...
	{
		server_print(" zombie_class %d    class %d", zombie_class, class)

		if (zombie_class != class)
		{
			if (!get_zclass_sound[class])
			{
				ArrayPushString(g_zclass_sound1, "0")
				ArrayPushString(g_zclass_sound2, "0")
				ArrayPushString(g_zclass_sound3, "0")
			}

			continue;
		}

		ArrayPushString(g_zclass_sound1, bhit)
		ArrayPushString(g_zclass_sound2, dieea)
		ArrayPushString(g_zclass_sound3, fall)

		get_zclass_sound[class] = true

		new buffer[32], i
		for (i = 0; i < ArraySize(g_zclass_sound1); i++)
		{
			ArrayGetString(g_zclass_sound1, i, buffer, charsmax(buffer))
			engfunc(EngFunc_PrecacheSound, buffer)
		}
		for (i = 0; i < ArraySize(g_zclass_sound2); i++)
		{
			ArrayGetString(g_zclass_sound2, i, buffer, charsmax(buffer))
			engfunc(EngFunc_PrecacheSound, buffer)
		}
		for (i = 0; i < ArraySize(g_zclass_sound3); i++)
		{
			ArrayGetString(g_zclass_sound3, i, buffer, charsmax(buffer))
			engfunc(EngFunc_PrecacheSound, buffer)
		}
		break;
	}
...
public native_get_user_godmode(id)
{
	return g_GoD_mode[id];
}

public native_set_user_godmode(id, godmode)
{
	// ZP disabled
	if (!g_pluginenabled)
		return;

	g_GoD_mode[id] = godmode
}
...
public burning_flame(taskid)
{
	...
	if (health - floatround(get_pcvar_float(cvar_firedamage), floatround_ceil) > 0)
		...
		if (get_pcvar_num(cvar_fire_kill)) fire_kill_player(ID_BURN, g_fire_ing[ID_BURN][1], origin)
...
public fire_kill_player(id, attacker, origin[3])
{
	// Smoke sprite
	message_begin(MSG_PVS, SVC_TEMPENTITY, origin)
	write_byte(TE_SMOKE) // TE id
	...
	ExecuteHamB(Ham_Killed, id, attacker, 1) // set last param to 2 if you want victim to gib
	set_msg_block(get_user_msgid("DeathMsg"), BLOCK_NOT)
	SendDeathMsg(attacker, id, 0, "Fire_Bomb")
...
SendDeathMsg(attacker, victim, hs, const weapon[])
{
	message_begin(MSG_BROADCAST, g_msgDeathMsg)
	write_byte(attacker) // killer
	write_byte(victim) // victim
	write_byte(hs) // headshot flag [1 or 0]
...
{
	set_user_info(ID_MODEL, "model", g_NSPY[ID_MODEL] ? g_NSPYmodel[ID_MODEL] : g_playermodel[ID_MODEL])
}
