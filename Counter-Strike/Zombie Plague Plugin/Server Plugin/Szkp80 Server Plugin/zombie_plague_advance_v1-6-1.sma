...
//new bool:OK_to_make
// For assassin
//new g_assassin_in_game[33] // 選定不幸的彊屍
//new bool:g_assassin_appear
//new bool:g_assassin_die
//new bool:OK_to_makea
// Game vars
...
public plugin_init()
{
	...
	// Format mod name
	formatex(g_modname, charsmax(g_modname), "【求生之路2】")
...
//	g_nemesis_die = false
//	OK_to_make = false
//	set_task(random_float(10.0,300.0), "make_rubbish_nemesis")
	// assassin Something
   // 	g_assassin_appear = false
  //  	g_assassin_die = false
    //	OK_to_makea = false
    //	set_task(random_float(10.0,300.0), "make_rubbish_assassin")
	// Remove doors/lights?
...
public fw_TakeDamage(victim, inflictor, attacker, Float:damage, damage_type)
{
	...
	// Nemesis/Assassin?
	if (g_nemesis[attacker] || (g_assassin[attacker] && !zps_assinfected()))
	{
	...
	if (g_assassin[attacker] && zps_assinfected())
	{
		if (fnGetHumans() != 1)
		{
			if (!zps_assinfarmor())
			{
				...
				if (armor > 0.0)
				{
					...
					set_pev(victim, pev_armorvalue, floatmax(0.0, armor - damage * 2))
					return HAM_SUPERCEDE;
				}
			}
			new assinfmod = zps_assinfected()

			if (assinfmod == 1) zombieme(victim, attacker, 0, 0, 1, 0) // turn into zombie
			else if (assinfmod == 2) zombieme(victim, attacker, 1, 0, 0, 0)
			else zombieme(victim, attacker, 0, 0, 0, 1)
			return HAM_SUPERCEDE;
		}
...
public fw_TakeDamage_Post(victim)
{
	...
	if (!g_nemesis[victim] && !g_assassin[victim])
	{
        ...
        remove_task(TASK_AMBIENCESOUNDS)
        set_task(0.1, "ambience_sound_effects", TASK_AMBIENCESOUNDS)
    ...
    //    	set_hudmessage(200, 0, 0, HUD_EVENT_X, HUD_EVENT_Y, 0, 0.0, 3.0, 2.0, 1.0, -1)
     ...
     //   set_hudmessage(200, 0, 0, HUD_EVENT_X, HUD_EVENT_Y, 0, 0.0, 3.0, 2.0, 1.0, -1)
     //   ShowSyncHudMsg(0, g_MsgSync, "assassin 已被殺死，喪屍將不可再重生。", LANG_PLAYER)
...
public fw_ClientUserInfoChanged(id)
{
	...
	if (!g_handle_models_on_separate_ent && (zps_ass_level(id) != 0 || zps_nem_level(id) != 0))
	{
		// Get current model
...
respawn_player_manually(id)
{
	...
	if (g_respawn_as_zombie[id])
	{
		...
		if(g_nemround && fnGetNemesis() == 0 || g_nemround && fnGetAssassin() == 0)
			return;

        	if (g_plagueround && fnGetNemesis() == 0 || g_plagueround && fnGetAssassin() == 0)
            		return;

		if (zps_nem_die() || zps_ass_die())
            		return;

        	new g_hahachance
		if (zps_round_nemlv2() || zps_round_nemlv1() || zps_round_asslv3() || zps_round_asslv2() || zps_round_asslv1())
			g_hahachance = 0
		else
			g_hahachance = random_num(1,5)

	//	OK_to_make = true
	//	OK_to_makea = true

        	if(fnGetAssassin() == 0 && fnGetNemesis() == 0 && g_hahachance == 1)
        	{
			zps_spawn_nemass(id)
/*
            		...
            		if(!g_assassin_appear && OK_to_makea && g_modestarted)
            		{
               			g_assassin_in_game[id] = true
                		g_assassin_appear = true
            		}
*/
			return;
		}
...
/*
    	...
    	if(g_assassin_in_game[id]) // 能活動就了召喚復仇者吧，嘿嘿嘿
    	{
        	g_assassin_appear = true
		remove_task(TASK_AMBIENCESOUNDS)
        	set_task(0.0, "ambience_sound_effects", TASK_AMBIENCESOUNDS)
